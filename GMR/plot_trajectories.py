...
gmm = GMM(
    ...
    random_state=random_state)  # 这个gmm是包含了时间t变量的
...
y_stds = []
for step in t:
    conditional_gmm = gmm.condition([0], np.array([step]))  # t为条件概率自变量
    conditional_mvn = conditional_gmm.to_mvn()  # 将条件GMM转化为（近似为）单高斯
...
if plot_covariances:
    ...
    for factor in np.linspace(0.5, 4.0, 8):
        new_gmm = GMM(
            ...
            random_state=gmm.random_state)  # 这个new_gmm是刨去时间t变量后的
